...
class Ersatzteil(Base):
    """Mappingklasse der DB-Tabelle Ersatzteil"""
    ...
    EtHersteller = Column('EtHersteller', String(50))

# Task 7.2

class Montage(Base):
    """Mappingklasse der DB-Tabelle Montage (verbaute Ersatzteile pro Auftrag)"""
    __tablename__ = 'Montage'

    EtId   = Column('EtID', String(10), ForeignKey('Ersatzteil.EtId'), primary_key=True, autoincrement=False)
    AufNr  = Column('AufNr', Integer, ForeignKey('Auftrag.Aufnr'), primary_key=True, autoincrement=False)
    Anzahl = Column('Anzahl', Integer)

    Ersatzteil = relationship('Ersatzteil')
